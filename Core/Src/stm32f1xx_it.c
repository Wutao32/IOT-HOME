...
void USART1_IRQHandler(void)
{
  ...
  if (LL_USART_IsActiveFlag_RXNE(USART1) == RESET)
  {
    ...
    if (strUSART_Fram_Record.InfBit.FramLength < (RX_BUF_MAX_LEN - 1))
      strUSART_Fram_Record.Data_RX_BUF[strUSART_Fram_Record.InfBit.FramLength++] = ch;
  }
  /* USER CODE END USART1_IRQn 0 */
  /* USER CODE BEGIN USART1_IRQn 1 */
  if (LL_USART_IsActiveFlag_IDLE(USART1) == SET) // 数据帧接收完成
  {
...
void USART2_IRQHandler(void)
{
  ...
  if (LL_USART_IsActiveFlag_RXNE(USART2) != RESET)
  {
    ...
    if (strEsp8266_Fram_Record.InfBit.FramLength < (RX_BUF_MAX_LEN - 1)) // 预留1字节写结束符
  ...
  if (LL_USART_IsActiveFlag_IDLE(USART2) == SET)
  {
    ...
    ch = LL_USART_ReceiveData8(USART2); // 获取连接状态
