...
{	
	MQTT_RxDataInPtr=MQTT_RxDataBuf[0]; 				 //指向接受缓冲区存放数据的指针归位
	MQTT_RxDataOutPtr=MQTT_RxDataInPtr; 				 //指向接受缓冲区读取数据的指针归位
	MQTT_RxDataEndPtr=MQTT_RxDataBuf[R_NUM-1];		     //指向接受缓冲区结束的指针归位
...
{	
	...
	sprintf(ServerIP,"%s",MQTTServerIP);              //构建服务器域名
    sprintf(SerVerPort,"%s",MQTTServerPort);           //构建服务器端口                      
...
void MQTT_DealPushdata_Qs0(unsigned char *redata)
{
    ...
    int  total_len;                        //定义一个变量，存放已经统计的推送的总数据量
	...
	pack_num = temp_len = total_len = temp = 0;           //各个变量清零
	local = 1;
	multiplier = 1;
	do{
		pack_num++;                                       //开始循环统计推送的个数，每次循环推送的个数+1	
		do{
			temp = data[total_len + local];   
			temp_len += (temp & 127) * multiplier;
			multiplier *= 128;
			local++;
		}while ((temp & 128) != 0);
		total_len += (temp_len + local);                  //累计统计的总的推送的数据长度
	...
	{                                		
		...
		topic_len = data[local + total_len]*256 + data[local + 1 + total_len] + 2; //计算本次推送数据中主题占用的数据量
		cmd_len = temp_len - topic_len;                              			   //计算本次推送数据中命令数据占用的数据量
		cmd_loca = total_len + local +  topic_len;                  			   //计算本次推送数据中命令数据开始的位置
		memcpy(tempbuff, &data[cmd_loca], cmd_len);                   			   //命令数据拷贝出来		                 
		CMDBuf_Deal(tempbuff, cmd_len);                             			   //加入命令到缓冲区
		total_len += (temp_len + local);                              			   //累计已经统计的推送的数据长度
		local = 1;
		multiplier = 1;
		temp_len = 0;
	}	
...
{
	memcpy(&MQTT_TxDataInPtr[2], data, size);     //拷贝数据到发送缓冲区，从index=2处开始放要发送的数据
	MQTT_TxDataInPtr[0] = size/256;               //记录数据长度
	MQTT_TxDataInPtr[1] = size%256;               //记录数据长度
	MQTT_TxDataInPtr += TBUFF_UNIT;               //指针下移
	if(MQTT_TxDataInPtr == MQTT_TxDataEndPtr)     //用于存放数据的指针到缓冲区尾部
		MQTT_TxDataInPtr = MQTT_TxDataBuf[0];     //指针归位到缓冲区开头，循环使用。
}
