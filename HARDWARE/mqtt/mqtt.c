...
{	
	...
	MQTT_Subscribe(S_TOPIC_LAMP,0);	                 	 //发送缓冲区添加订阅topic，等级0	
	MQTT_Subscribe(S_TOPIC_DHT,0);
...
{	
	...
	sprintf(ServerIP,"%s",SERVERIP);              //构建服务器域名
	ServerPort = SERVERPORT;                                   //服务器端口号6002
...
void MQTT_DealPushdata_Qs0(unsigned char *redata)
{
  ...
  int  totle_len;                        //定义一个变量，存放已经统计的推送的总数据量
	...
	//存放主题
//	unsigned char tbuff[50];
	//存放命令
	//unsigned char cbuff[30];
	int topic_loca; //主题位置
	...
	{                                		
		...
		topic_len = data[local + totle_len]*256 + data[local + 1 + totle_len]; //计算本次推送数据中主题占用的数据量
		//u1_printf("topic_len:%d\r\n",topic_len);
		cmd_len = temp_len - (topic_len + 2);                              			   //计算本次推送数据中命令数据占用的数据量
		//u1_printf("cmd_len:%d\r\n",cmd_len);
		cmd_loca = totle_len + local +  topic_len + 2;                  			   //计算本次推送数据中命令数据开始的位置
		//u1_printf("cmd_loca:%d\r\n",cmd_loca);
		topic_loca = cmd_loca - (topic_len);
		//u1_printf("topic_loca:%d\r\n",topic_loca); 
		//memcpy(tbuff, &data[topic_loca], topic_len);
		//u1_printf("收到的主题:%s主题长度:%d\r\n",tbuff,topic_len);
		//memcpy(cbuff, &data[cmd_loca], cmd_len);                   			   //命令数据拷贝出来
		//u1_printf("收到的命令:%s命令长度:%d\r\n",cbuff,cmd_len);
		
		//先放主题再放命令
		memcpy(tempbuff, &data[topic_loca], topic_len);
		memcpy(&tempbuff[topic_len], &data[cmd_loca], cmd_len);
		
		u1_printf("收到命令%s\r\n",tempbuff);
		CMDBuf_Deal(tempbuff, topic_len+cmd_len);                             			   //加入命令(主题+命令)到缓冲区
